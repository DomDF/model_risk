...
using Distributions, Random, Statistics, Parameters
using Flux, BSON
using Plots, DataFrames, ProgressMeter

# Configuration parameters
const n_assets = 5  # Increased number of assets
const n_asset_types = 3  # Different types of assets
const n_condition_levels = 4  # More granular condition levels
const max_history_length = 5  # How many past observations to retain
...
# Define environmental conditions that affect asset operation
@enum Weather NORMAL HARSH EXTREME
...
struct FleetPOMDP <: POMDP{FleetState, FleetAction, FleetObservation}
    ...
    weather_transition::Matrix{Float64}  # Weather transition probabilities
    capacity_growth_rate::Float64  # Rate at which maintenance capacity replenishes
end

# Helper function to calculate failure probability based on condition, type, usage and weather
function failure_probability(condition::Int, asset_type::Int, 
                            usage_hours::Float64, time_since_maintenance::Float64,
                            weather::Weather)
    # Base probability depends on condition
    base_prob = (condition / n_condition_levels)^2
    
    # Adjust for asset type's failure rate
    type_factor = ASSET_TYPES[asset_type].base_failure_rate
    
    # Usage factor increases failure probability
    usage_factor = 1.0 + (usage_hours / 1000.0)^1.5
    
    # Maintenance factor - increases failure probability as time since maintenance increases
    maintenance_factor = 1.0 + 0.1 * log(1.0 + time_since_maintenance / 100.0)
    
    # Weather factor - harsh conditions increase failure probability
    weather_factor = weather == NORMAL ? 1.0 : 
                    (weather == HARSH ? 1.3 : 1.8)  # EXTREME case
    
    # Combine all factors, ensuring probability stays in [0,1]
    prob = min(base_prob * type_factor * usage_factor * maintenance_factor * weather_factor, 1.0)
    
    return prob
end

# Function to degrade condition based on usage and weather
function degrade_condition(condition::Int, asset_type::Int, 
                          usage_increment::Float64, weather::Weather)
    # Base degradation depends on asset type
    base_degradation = ASSET_TYPES[asset_type].base_failure_rate * usage_increment / 50.0
    
    # Weather factor
    weather_factor = weather == NORMAL ? 1.0 : 
                    (weather == HARSH ? 1.4 : 2.0)  # EXTREME case
    
    # Combine factors
    degradation_prob = base_degradation * weather_factor
    
    # Probabilistic degradation
    if rand() < degradation_prob
        return min(condition + 1, n_condition_levels)
    else
        return condition
    end
end
...
function generate_observation(state::FleetState)
    ...
    warning_indicators = ntuple(i -> begin
        warning_prob = (state.conditions[i] / n_condition_levels)^1.5
        return rand() < warning_prob
    end, n_assets)
    
    # Default inspection results (no recent inspection)
    default_inspection = ntuple(i -> 0, n_assets)
...
# Transition function - given current state and action, generate next state
function POMDPs.transition(pomdp::FleetPOMDP, s::FleetState, a::FleetAction)
    ...
    # 1. Update weather according to transition matrix
    weather_idx = Int(s.weather) + 1  # Convert enum to index
    next_weather_idx = sample(1:3, weights(pomdp.weather_transition[weather_idx, :]))
    next_weather = Weather(next_weather_idx - 1)  # Convert back to enum
    
    # 2. Update maintenance capacity
    ...
    # 3. Update usage hours (different assets might be used more/less)
    usage_increment = ntuple(i -> rand(10.0:30.0), n_assets)
    next_usage_hours = ntuple(i -> s.usage_hours[i] + usage_increment[i], n_assets)
    
    # 4. Update time since maintenance
    next_time_since_maintenance = ntuple(i -> 
        updated_time_since_maintenance[i] + 1.0, n_assets)
    
    # 5. Degrade conditions based on usage and weather
    final_conditions = ntuple(i -> 
        degrade_condition(updated_conditions[i], updated_asset_types[i], 
                         usage_increment[i], s.weather), 
        n_assets)
...
function create_fleet_pomdp()
    # Weather transition matrix: rows=current, cols=next
    # [NORMAL, HARSH, EXTREME]
    weather_transition = [
        0.7 0.25 0.05;   # From NORMAL
        0.3 0.6 0.1;     # From HARSH
        0.2 0.5 0.3      # From EXTREME
    ]
...
# Deep Recurrent Q-Network Agent for the POMDP
struct DRQNAgent
    qnetwork::Chain                # Q-network with recurrent layer
    replay_buffer::Vector{Any}     # Experience replay buffer
    update_freq::Int               # How often to update the network
    ...
    hidden_size::Int               # Size of recurrent hidden state
    learning_rate::Float64         # Learning rate
    buffer_size::Int               # Maximum buffer size
    optimizer                      # Optimizer
    current_step::Int              # Current training step
    obs_history::Vector{Any}       # History of observations
    action_space::Vector{FleetAction}  # All possible actions
end

# Create DRQN agent
function create_drqn_agent(pomdp::FleetPOMDP)
    ...
    # Define feature size
    feature_size = 4 * n_assets + 3  # Number of features extracted from observation
    hidden_size = 128
    
    # Create Q-network with LSTM layer
    qnetwork = Chain(
        Dense(feature_size, hidden_size, relu),
        LSTM(hidden_size, hidden_size),
        Dense(hidden_size, n_actions)
    )
...
function extract_features(obs::FleetObservation)
    ...
    # Asset types (one-hot encode)
    # This would make the feature vector very large, so we'll just use the type index
    append!(features, collect(obs.asset_types) ./ n_asset_types)
    
    # Weather (one-hot encode)
    weather_idx = Int(obs.weather)
    weather_features = zeros(Float32, 3)
    weather_features[weather_idx+1] = 1.0
    append!(features, weather_features)
...
function select_action(agent::DRQNAgent, obs::FleetObservation, evaluate::Bool=false)
    # Extract features
    features = extract_features(obs)
    
    # Add to history
    push!(agent.obs_history, features)
    
    # Keep only the last max_history_length observations
    if length(agent.obs_history) > max_history_length
        popfirst!(agent.obs_history)
    end
    ...
    # Prepare batch of history - pad if needed
    if length(agent.obs_history) < max_history_length
        # Pad with zeros
        padded_history = vcat([zeros(Float32, length(features)) for _ in 1:(max_history_length-length(agent.obs_history))], 
                             agent.obs_history)
    else
        padded_history = agent.obs_history
    end
    
    # Convert to batch format
    history_tensor = reshape(hcat(padded_history...), :, 1)
    
    # Forward pass through Q-network
    q_values = agent.qnetwork(history_tensor)
...
function store_experience(agent::DRQNAgent, obs_history, action_idx, reward, next_obs, done)
    # Store the sequence as a whole
    experience = (obs_history, action_idx, reward, next_obs, done)
...
# Training function for DRQN agent
function train(agent::DRQNAgent)
    ...
    histories = [exp[1] for exp in batch]
    actions = [exp[2] for exp in batch]
    rewards = [exp[3] for exp in batch]
    next_obss = [exp[4] for exp in batch]
    ...
    loss, grads = Flux.withgradient(agent.qnetwork) do q
        ...
        for i in 1:agent.batch_size
            # Get the history sequence
            history = histories[i]
            
            # Forward pass through Q-network
            history_tensor = reshape(hcat(history...), :, 1)
            q_values = q(history_tensor)
            ...
            if dones[i]
                ...
                next_obs = next_obss[i]
                
                # Create a new history including this observation
                next_history = vcat(history[2:end], [next_obs])
                
                # Forward pass for next state
                next_history_tensor = reshape(hcat(next_history...), :, 1)
                next_q_values = q(next_history_tensor)
                
                # Target is reward plus discounted max Q-value
                target = rewards[i] + agent.γ * maximum(next_q_values)
            end
            
            # Add to total loss
            total_loss += (q_value - target)^2
        end
        
        return total_loss / agent.batch_size
    end
...
# Full training loop
function train_drqn(pomdp::FleetPOMDP, agent::DRQNAgent, n_episodes::Int)
    # Initialize metrics
    rewards_history = []
    loss_history = []
    
    # Progress meter
    progress = Progress(n_episodes, dt=1.0, desc="Training DRQN... ", barglyphs=BarGlyphs("[=> ]"))
    ...
    for episode in 1:n_episodes
        ...
        while !done && step < 100  # Cap at 100 steps
            ...
            current_history = copy(agent.obs_history)
            ...
            # Store experience
            store_experience(agent, current_history, action_idx, r, extract_features(op), done)
            
            # Update agent
            agent.current_step += 1
            if agent.current_step % agent.update_freq == 0
                loss = train(agent)
...
# Evaluation function
function evaluate_drqn(pomdp::FleetPOMDP, agent::DRQNAgent, n_episodes::Int=20)
    rewards = []
...
function analyze_policy(pomdp::FleetPOMDP, agent::DRQNAgent)
    ...
    test_states = [
        ...
        # Mixed condition fleet in harsh weather
        FleetState(ntuple(i -> rand(1:n_condition_levels), n_assets), ntuple(i -> rand(1:n_asset_types), n_assets),
                 ntuple(i -> rand() * 100.0, n_assets), ntuple(i -> rand() * 50.0, n_assets),
                 HARSH, 0.8, 20),
        
        # Poor condition fleet in extreme weather
        ...
        # Very poor condition fleet in normal weather
...
function main()
    ...
    # Create DRQN agent
    agent = create_drqn_agent(pomdp)
    
    # Train agent
    println("Starting training...")
    rewards, losses = train_drqn(pomdp, agent, 1000)
    
    # Evaluate agent
    mean_reward, std_reward = evaluate_drqn(pomdp, agent, 50)
    ...
    @save "fleet_drqn_model.bson" model_state
